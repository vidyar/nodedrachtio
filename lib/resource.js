...
function Resource(id) {
	...
	this.__defineGetter__('sessionID', function(){
		return (self.constructor.prefix || '') + self._id;
	});

}

Resource.prototype.attachSession = function(mks) {
	if( this.hasOwnProperty('mks') ) return ;
	
	var MultiKeySession = require('./middleware/session/multikeysession') ;
	if( !(mks instanceof MultiKeySession ) ) throw new Error('Resource#attachSession: arg must be an instance of MultiKeySession') ;

    Object.defineProperty( this, 'mks', {value:mks}) ;
