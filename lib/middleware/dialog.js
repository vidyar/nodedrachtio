...
module.exports = function dialog() {
    ...
    return function(req, res, next) {
        ...
        if( req.source === 'network' && !req.sessionStore ) return next('drachtio.dialog() requires drachtio.session() middleware to be installed prior') ;
        ...
        if( req.isNewInvite() ) {
            if( req.source === 'network' ) {
                ...
                res.send = function( code, status, opts ) {
                    ...
                    debug('proxying res.send with status code %d', code) ;
                ...
                switch( req.method ) {
                  ...
                  case 'PRACK':
                    if( (dialog.state === SipDialog.PENDING || dialog.state === SipDialog.EARLY) && req.source === 'network') {
                        ...
                        dialog.local.tag = req.get('to').tag ;
                        emit( 'sipdialog:create-early', {
                          target: dialog
                          ,req: req
                          ,res: res
                        }) ;

                        /* there is no response to an PRACK, so we need to save it ourselves */
                        dialog.save( req.sessionStore ) ;                            
                    }
                    break ;
