...
MultiKeySession.prototype.save = function( cb ) {
	...
	function dessicate( obj, liveSpecimens ) {
		if( _.isFunction(obj) ) return ;
		
		if( _.isArray( obj ) ) _.each( obj, function( el, idx ) { dessicate( el, liveSpecimens ) ; }) ;
		...
		if( _.isObject( obj ) ) {
			_.each( obj, function( v, k, l) {
				...
				else dessicate(v, liveSpecimens) ;
			}) ;
		}
		return ;
	}
	...
	function undessicate( obj, liveSpecimens ) {
		if( _.isFunction(obj) ) return ;
		if( _.isArray( obj ) ) _.each( obj, function( el, idx ) { undessicate( el, liveSpecimens ) ; }) ;
		...
		if( _.isObject( obj ) ) {
			_.each( obj, function( v, k, l ) {
				...
				else undessicate(v, liveSpecimens) ;
			}) ;
		}
	}
	...
	async.parallel([
		...
		, function saveMKS(callback) {
			...
			dessicate( mks.session, liveSpecimens ) ;

			(function(lv) {
				self._store.set( self._uuid, mks, function(err, res) {
					undessicate(mks.session, lv) ;
					if( err ) return callback(err) ;
					callback(null, res) ;
				}) ;
			})(liveSpecimens) ;
