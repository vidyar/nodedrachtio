 ...
 */
MultiKeySession.prototype.add = function( resource ) {

	if( !(resource instanceof Resource) ) throw new Error('MultiKeySession#add: object to be added as key must have Resource in its prototype chain') ;

	var sessionID = resource.sessionID ;
...
MultiKeySession.prototype.save = function( cb ) {
	...
	function dessicate( obj ) {
		debug('prepare obj: ', obj) ;
		if( _.isFunction(obj) ) {
			debug('ignoring function object');
			return ;
		}
		...
		if( _.isObject( obj ) ) {
			_.each( obj, function( v, k, l) {
				debug('object constructor for key %s is ', k, v.__proto__ ? v.__proto__.constructor.name: 'null') ;
				if( _.isObject(v) && !_.isFunction(v) && v.__proto__ !== null && v.__proto__.constructor.name !== 'Object') {
					debug('need to sub in an object here')
	 ...
	 * so the caller doesn't get unanticipated side affects
	 */
	...
	async.parallel([
		function saveKeys(callback){
			...
			async.each( array, function(key, cb) {
				...
				self._store.set( prefix + key.sessionID, self.uuid, function(err, res) {
		...
		, function saveMKS(callback) {
			...
			debug('before dessicate ', mks.session) ;
			dessicate( mks.session ) ;
			debug('after dessicate ', mks.session) ;

			self._store.set( self._uuid, mks, function(err, res) {
				undessicate(mks.session) ;
				debug('after undessicate ', mks.session) ;
				if( err ) return callback(err) ;
				callback(null, res) ;
			}) ;
		}
...
MultiKeySession.prototype.destroy = function(cb) {
	...
	/* remove the keys */
	debug('destroy keyData ', this._keyData) ;
...
MultiKeySession.loadFromStorage = function( opts, id, cb ) {
	...
	debug('resolverMap ', resolverMap) ;

	function hydrate( obj ) {
		debug('hydrate obj: ', obj) ;
		if( _.isFunction(obj) ) {
			debug('ignoring function object');
			return ;
		}
		...
		if( _.isObject( obj ) ) {
			_.each( obj, function( v, k, l ) {
				if( _.isObject(v) && '__constructorName__' in v && '__instanceData__' in v ) {
					debug('got an object to try to hydrate: %s', k);
					var object = v['__instanceData__'] ;
					if( v['__constructorName__'] in resolverMap ) {
						debug('calling constructor for %s', v['__constructorName__'])
						var F = resolverMap[v['__constructorName__']] ;
						object = new F(object) ;
						debug('constructed object: ', object)
					}
					else {
						debug('couldnt resolve function %s', v['__constructorName__']) ;
						debug('object will be ', object)
					}
					l[k] = object ;
				} 
				else {
	...
	store.get( prefix + id, function(err, uuid) {
		...
		store.get( uuid, function( err, mks ) {
			...
			_.each( mks.keys, function( v, k ){
				...
				keyData[k] = {} ;
				keyData[k].resource = F ? new F(v.instanceData) : v.instanceData;
				keyData[k].saved = true ;
			}) ;

			debug('hydrate object ', mks.session) ;
			hydrate( mks.session ) ;
			debug('hydrated object ', mks.session) ;
