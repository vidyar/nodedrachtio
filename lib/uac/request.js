...
function Request( options ) {
	...
	if( 'session' in options ) {
		...
		delete this['session'] ;
		var sess = options['session'] ;
		delete options['session'] ;
		if( sess instanceof MultiKeySession.SessionProto) Object.defineProperty(this,'mks', {value: sess.mks}) ;
		throw new Error('Request: session object provided options is not an instanceof MultiKeySession.SessionProto') ;
	}
...
Request.prototype.pipe = function( res, opts ) {
	...
	function callback( err, invite, uacRes ) {
      if( err ) throw( err ) ;
        ...
        else if( 200 === uacRes.statusCode ) b2bua.emit('success') ;
	}
