	...
	*/
	...
	if( code >= 200 ) {
		...
		if( this.req.method === 'INVITE' ) {
			this.agent.removeCancelableTransaction( req.transactionId ) ;
		}
	}

	var callbacks = (200 == code && this.req.isNewInvite) ? _.flatten( args.slice( cbStart ) ) : [];
	...
	if( callbacks.length > 0  && code === 200 ) {
...
Response.prototype.ack = function( opts, callback ) {
	...
	process.nextTick( function() {
		callback.call( process.root, self.dialog ) ;
	}) ;
