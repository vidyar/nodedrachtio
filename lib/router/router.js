...
Router.prototype._dispatch = function(err, req, res, next){
  ...
  (function pass(i, err){
    ...
    else if( dialog && method === 'prack' && !dialog.times.connect_time && dialog.ackCallbacks ) {
      /* PRACK to a reliable response, invoke middleware in Res.send( 183, ... ) */
      req.route = route = {method: 'dialog-ack-construction', callbacks: dialog.ackCallbacks };
      debug('found dialog-level route for PRACK that completes early dialog constructions') ; 
      res = dialog ;  //special case for PRACK callback; fn( req, ack) instead of fn(req, res)    
    }
