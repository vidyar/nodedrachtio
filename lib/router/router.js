...
Router.prototype.dispatchDialogEvent = function( dialogId, eventName, eventData ) {
  if( dialogId in this.mapDialog ) {
    debug('emitting event %s for dialog %s: ', eventName, dialogId ) ;
    this.mapDialog[dialogId].emit( eventName, eventData ) ;

    if( 'terminate' === eventName ) {
      delete this.mapDialog[dialogId] ;
      debug('removing dialog, count of dialog is now %d', _.size( this.mapDialog ) ) ;
    }
  }
  else {
    debug('dialogId %s not found, count of dialogs is %d', dialogId, _.size( this.mapDialog )) ;
  }
}
/**
